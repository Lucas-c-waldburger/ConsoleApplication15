...
//	{
//		auto handle = Handle<EventCallback>::Create(T::GetEventType());
//
//		map_[T::GetEventType()].emplace_back(EventCallback{
//			.onEvent = std::forward<Fn>(fn),
//			.handle = handle
//		});
//
//		return handle;
...
//
//	//bool SetSignal(const Handle<EventCallback>& handle, ReturnSignal signal);
//
...
//
//private:
//	using CallbackIter = std::vector<EventCallback>::iterator;
//	static inline CallbackIter kCallbacksEnd = std::vector<EventCallback>{}.end();
//
//	CallbackIter FindCallback(const Handle<EventCallback>& handle);
//
