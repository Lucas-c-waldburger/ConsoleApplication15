...
Result<Void> Lua::Run()
{
	...
	case ScriptType::File:
		if (scriptInfo_.path.empty())
		{
			return MAKE_ERROR("Filepath for script marked ScriptType::File was empty");
		}

		ret = lua_.script_file(scriptInfo_.path, sol::script_pass_on_error);
		break;
